var Redmon = (function() {
  ...
  /**
  * Returns current redis param
  */
  function redis_param() {
    var redis = $('.pull-right a.brand').text()
    if (redis.length > 0) {
      redis = '&redis=' + redis
    };
    return redis
  ...
  function requestData(count, callback) {
    $.ajax({
      url: 'stats?count='+count+redis_param(),
  ...
  var toolbar = (function() {
    ...
    function onBtnClick(cmd) {
      $.ajax({url: 'cli?command='+cmd+redis_param()});
    }
  ...
  var cli = (function() {
    ...
    function process(command, callback) {
      ...
      $.ajax({
        url     : 'cli?command='+command+redis_param(),
        success :  callback
      });
    }
